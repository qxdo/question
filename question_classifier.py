...
class QuestionClassifier:
    def __init__(self):
        ...
        self.symptoms_path = os.path.join(cur_dir, 'dict/symptom.txt')
        ...
        self.region_words = set(self.department_wds + self.disease_wds + self.check_wds + self.drug_wds + self.food_wds + self.producer_wds + self.symptoms_wds)
        ...
        # 问句疑问词
        self.symptoms_qwds = ['症状', '表征', '现象', '症候', '表现']
    ...
    def classify(self, question):
        ...
        # 症状
        if self.check_words(self.symptoms_qwds, question) and ('disease' in types):
            question_type = 'disease_symptoms'
            question_types.append(question_type)

        if self.check_words(self.symptoms_qwds, question) and ('symptoms' in types):
        ...
        # 若没有查到相关的外部查询信息，那么则将该疾病的描述信息返回
        if question_types == [] and 'symptoms' in types:
            question_types = ['symptoms_disease']
