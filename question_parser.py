...
class QuestionPaser:
    ...
    def sql_transfer(self, question_type, entities):
        ...
        # 查询疾病有哪些症状
        elif question_type == 'disease_symptoms':
            sql = ["MATCH (m:Disease)-[r:has_symptoms]->(n:symptoms) where m.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]

        # 查询症状会导致哪些疾病
        elif question_type == 'symptoms_disease':
            sql = ["MATCH (m:Disease)-[r:has_symptoms]->(n:symptoms) where n.name = '{0}' return m.name, r.name, n.name".format(i) for i in entities]
